...
class ir_mail_server(osv.osv):
    ...
    def _garbage_collect_sent_emails(self, cr, uid, context=None):
        """ Garbage collect unused cron jobs created by send_email_silent()
        """
        ir_cron_obj = self.pool.get('ir.cron')
        cron_ids = ir_cron_obj.search(cr, uid, [
                            ('model', '=', 'ir.mail_server'),
                            ('active', '=', 0),
                            ], context=context)
        if cron_ids:
            ir_cron_obj.unlink(cr, uid, cron_ids, context=context)

        return True

    def send_email_silent_from_cron(self,cr, uid,smtp_server,smtp_port,smtp_user,smtp_password,smtp_encryption,smtp_debug,smtp_from, smtp_to_list, message):
        smtp = None
        smtp = self.connect(smtp_server,
                            ...
                            smtp_debug)
        smtp.sendmail(smtp_from, smtp_to_list, message)
        if smtp is not None:
            smtp.quit()
        return True

    def send_email_silent(self,cr, uid,smtp_server,smtp_port,smtp_user,smtp_password,smtp_encryption,smtp_debug, smtp_from,smtp_to_list,message):
        self.pool['ir.cron'].create(cr, uid, {
            'name': 'Send Emails Background',
            'user_id': uid,
            'priority': 1,
            'model': 'ir.mail_server',
            'function': 'send_email_silent_from_cron',
            'args': repr([smtp_server,smtp_port,smtp_user,smtp_password,smtp_encryption,smtp_debug,smtp_from, smtp_to_list, message])
        })
        return True
